<?php
/**
 * DynamicProgressionAcceptance Class | DynamicProgressionAcceptance.php
 * 
 * This class generate additional tag in the Contact Form 7 menu : Progression Acceptance
 * the input generated prevent the submission of the survey if the terms&conditions aren't accepted
 * 
...
class DynamicProgressionAcceptance {
	 ...
	 */
	public function addProgressionAcceptance() {
		// add new tag to the Contact Form 7 menu
		$this->hookManager->addHook( 'action', 'wpcf7_init', $this, 'wpcf7_add_form_tag_dynamic_acceptance', 10, 0 );
        $this->hookManager->addHook( 'filter', 'wpcf7_validate_dynamic_select', $this, 'wpcf7_dynamic_acceptance_validation_filter', 10, 2 );
        $this->hookManager->addHook( 'filter', 'wpcf7_validate_dynamic_select', $this, 'wpcf7_dynamic_acceptance_filter', 10, 2 );
		$this->hookManager->addHook( 'filter', 'wpcf7_validate_dynamic_select', $this, 'wpcf7_dynamic_acceptance_form_class_attr', 10, 1 );
		$this->hookManager->addHook( 'filter', 'wpcf7_validate_dynamic_select', $this, 'wpcf7_dynamic_acceptance_mail_tag', 10, 4 );
		$this->hookManager->addHook( 'action', 'wpcf7_admin_init', $this, 'wpcf7_add_tag_generator_dynamic_acceptance', 35, 0 );
	}
    ...
    function wpcf7_add_form_tag_dynamic_acceptance() {
        wpcf7_add_form_tag( 'dynamic_acceptance',
            [ $this, 'wpcf7_dynamic_acceptance_form_tag_handler' ],
            array(
                'name-attr' => true,
            )
        );
    ...
    function wpcf7_dynamic_acceptance_form_tag_handler( $tag ) {
        if ( empty( $tag->name ) ) {
            return '';
        }
...
//end Dynamic-Progression   
    ...
    /* Validation filter */
    //add_filter( 'wpcf7_validate_acceptance','wpcf7_dynamic_acceptance_validation_filter', 10, 2 );

    function wpcf7_dynamic_acceptance_validation_filter( $result, $tag ) {
    ...
    /* Acceptance filter */
    // add_filter( 'wpcf7_acceptance', 'wpcf7_dynamic_acceptance_filter', 10, 2 );

    function wpcf7_dynamic_acceptance_filter( $accepted, $submission ) {
        $tags = wpcf7_scan_form_tags( array( 'type' => 'dynamic_acceptance' ) );
    ...
    // add_filter( 'wpcf7_form_class_attr', 'wpcf7_dynamic_acceptance_form_class_attr', 10, 1 );
    function wpcf7_dynamic_acceptance_form_class_attr( $class ) {
        if ( wpcf7_acceptance_as_validation() ) {
    ...
    // add_filter( 'wpcf7_mail_tag_replaced_acceptance', 'wpcf7_dynamic_acceptance_mail_tag', 10, 4 );
    function wpcf7_dynamic_acceptance_mail_tag( $replaced, $submitted, $html, $mail_tag ) {
    ...
    // add_action( 'wpcf7_admin_init', 'wpcf7_add_tag_generator_dynamic_acceptance', 35, 0 );

    function wpcf7_add_tag_generator_dynamic_acceptance() {
        $tag_generator = WPCF7_TagGenerator::get_instance();
        $tag_generator->add( 'dynamic_acceptance', __( 'Progression Acceptance', 'contact-form-7' ),
            [ $this, 'wpcf7_tag_generator_dynamic_acceptance' ]);
    }

    function wpcf7_tag_generator_dynamic_acceptance( $contact_form, $args = '' ) {
        $args = wp_parse_args( $args, array() );
        $type = 'dynamic_acceptance';
