<?php
/**
 * DynamicProgressionCheckboxes Class | DynamicProgressionCheckboxes.php
 * 
 * This class generate some additional tag in the Contact Form 7 menu
 * 
 * The plugin preserves the way Contact Form 7 produce his tag. 
 * Dynamic-Select : The part changed here is the choice of a dynamic value (as first option)  replacing the "blank_value" in the regular CF7 select-menu
 * 
...
class DynamicProgressionCheckboxes {
    function __construct() {
        add_action( 'wpcf7_init', [$this, 'wpcf7_add_form_tag_dynamic_checkbox'], 10, 0 );
        add_filter( 'wpcf7_validate_dynamic_checkbox', [$this, 'wpcf7_dynamic_checkbox_validation_filter'], 10, 2 );
        add_filter( 'wpcf7_validate_dynamic_checkbox*', [$this, 'wpcf7_dynamic_checkbox_validation_filter'], 10, 2 );
        add_action( 'wpcf7_admin_init', [$this, 'wpcf7_add_tag_generator_dynamic_checkbox_and_radio'], 30, 0 );
        //add_filter( 'wpcf7_posted_data', [$this, 'wpcf7_dynamic_checkbox_posted_data'], 10, 1 );
    }
    /**
    ** A custome module for [checkbox], [checkbox*], and [radio]
    **/
    ...
    function wpcf7_add_form_tag_dynamic_checkbox() {
        wpcf7_add_form_tag( array( 'dynamic_checkbox', 'dynamic_checkbox*' ),
            [$this, 'wpcf7_dynamic_checkbox_form_tag_handler'],
            array(
                'name-attr' => true,
    ...
    function wpcf7_dynamic_checkbox_form_tag_handler( $tag ) {
        if ( empty( $tag->name ) ) {
            return '';
        }
        ...
        //Etienne $free_text = $tag->has_option( 'free_text' );
        $multiple = false;
        if ( 'dynamic_checkbox' == $tag->basetype ) {
            $multiple = ! $exclusive;
        } else { // radio
            $exclusive = false;
        }
        ...
        foreach ( $values as $key => $value ) {
            ...
            //Etienne retourner un attribut type checkbox pour le html
            $tag->basetype = 'checkbox';

            //dynamic_value has to be checked during the tag creation, user has to be logged in
            if (  is_user_logged_in() )
            {
                //$this->CF7DP_select_modification(); since object isn't an option
                require_once( CF7DP_PATH . 'inc/DynamicProgressionData.php' );
                global $wpdb;
                global $post;
                $user = wp_get_current_user();
                $cf7dp_datas = new DynamicProgressionData( $wpdb, $user, $post );
                $dynamic_val = $cf7dp_datas->dynamicProgGetResults( $tag->name );

                //dynamic-checkbox can be explicitly "exclusive" ($dynamic_val is a string), or by default "multiple choice" ($dynamic_val is an array)
                if (is_array($dynamic_val)) {
                    // if current input $value have been store : checked
                    if (in_array($value, $dynamic_val)) {
                        $checked = true;
                    }
                // if checkbox is exclusive $dynamic_val is a string
                } else if (strpos($dynamic_val, $value) !== false)  {
                    $checked = true;                               
                }
            }
            $item_atts = array(
    ...
    function wpcf7_dynamic_checkbox_validation_filter( $result, $tag ) {
        $name = $tag->name;
    ...
    function wpcf7_dynamic_checkbox_posted_data( $posted_data ) {
        $tags = wpcf7_scan_form_tags(
            array( 'type' => array( 'dynamic_checkbox', 'dynamic_checkbox*' ) ) );
    ...
    function wpcf7_add_tag_generator_dynamic_checkbox_and_radio() {
        if ( ! class_exists( 'WPCF7_TagGenerator' ) )
		{
			return;
		}
        $tag_generator = WPCF7_TagGenerator::get_instance();
        $tag_generator->add( 'dynamic_checkbox', __( 'Progression checkboxes', 'contact-form-7' ), [$this, 'wpcf7_tag_generator_dynamic_checkbox'] );
    }

    function wpcf7_tag_generator_dynamic_checkbox( $contact_form, $args = '' ) {
